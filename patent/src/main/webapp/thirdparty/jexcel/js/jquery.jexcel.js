...
var methods = {
            ...
            $.fn.jexcel.contextMenuControls = function (e) {
                ...
                if ($.fn.jexcel.current) {
                    ...
                    if ($(table).is('.jexcel')) {
                        ...
                        if (o) {
                            var size = $(e.target).closest("tbody").children().length - 1;
                            o = o.split('-');
                            contextMenuContent = '';
                            // Custom context menu
                            if (typeof($.fn.jexcel.defaults[$.fn.jexcel.current].contextMenu) == 'function') {
                                ...
                                if ($(e.target).parent().parent().is('tbody')) {
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertRow == true) {
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertRow', 1, " + o[1] + ")\">插入新行<span></span></a><hr>";
                                    }
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteRow == true) {
                                      if($($(e.target).parent()).hasClass('.insert-new')){
                                          contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('deleteRow'," + o[1] + ")\">删除当前行<span></span></a><hr>";
                                      }else{
                                          contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('hideRow'," + o[1] + ")\">删除当前行<span></span></a><hr>";
                                      }
                                    }
                                    //contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('download')\">Save as...<span>Ctrl + S</span></a>";
                                    if ( o[1]>0 ){
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('moveRow', " + o[1] + ", 0)\">上移到顶部</a>";
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('moveRow', " + o[1] + ", " + (parseInt(o[1]) - 1) + ")\">上移</a>";
                                    }
                                    if ( o[1] < size ){
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('moveRow', " + o[1] + ", " + (parseInt(o[1]) + 1) + ")\">下移</a>";
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('moveRow', " + o[1] + ", " + size + ")\">下移到底部</a>";
                                    }
                                }
                            }
    ...
    getData : function(highlighted) {
        ...
        var x = $(this).find('thead tr td').not(':first').length+1;
    ...
    hideRow : function(rowNumber, numOfRows){
        // Id
        var id = $(this).prop('id');
        // Main configuration
        var options = $.fn.jexcel.defaults[id];
            ...
            if (options.data.length > 1) {
                if (parseInt(rowNumber) > -1) {
                    ...
                    for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
                        ...
                        var last_td=$(line).find('td:last').attr('id');
                        var str=last_td.split('-');
                        var deleteid=(parseInt(str[0])+1)+'-'+str[1];
                        var td ='<td id="'+deleteid+'" style="display: none;">D</td>';
                        $(line).append(td);
                        $(line).hide();
                    }
                    // Remove from source
                    var a=$.fn.jexcel.defaults[id].data[parseInt(rowNumber)];
                    a.push('D');
                    $.fn.jexcel.defaults[id].data.splice(parseInt(rowNumber), numOfRows,a);
                    // Remove table references
                    $(this).jexcel('updateTableReferences', 0, rowNumber);
