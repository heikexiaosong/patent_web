(function ($) {
    ...
    myflow.rect = function (o, r, id) {
        var _this = this,
            _o = $.extend(true, {}, myflow.config.rect, o), _id = id || 'rect' + myflow.util.nextId(),
            _r = r, // Raphael画笔
            _rect, _img, _name, _text,
            _ox, _oy; // 拖动时，保存起点位置;

        var attr = _o.attr;
        _rect = _r.rect(attr.x, attr.y, attr.width, attr.height, attr.r).hide().attr(attr);

        _img = _r.image(myflow.config.basePath + _o.img.src, attr.x + _o.img.width / 2, attr.y + (attr.height - _o.img.height) / 2, _o.img.width, _o.img.height).hide();
        _name = _r.text(attr.x + _o.img.width + (attr.width - _o.img.width) / 2, attr.y + myflow.config.lineHeight / 2, _o.name.text).hide().attr(_o.name);
        _text = _r.text(attr.x + _o.img.width + (attr.width - _o.img.width) / 2, attr.y + (attr.height - myflow.config.lineHeight) / 2 + myflow.config.lineHeight, _o.text.text).hide().attr(_o.text); // 文本
        ...
        _text.drag(dragMove, dragStart, dragUp);
        ...
        this.toJson = function () {
            var data = "{etype:'" + _o.etype + "',ID:'" + (!_o.ID ? "" : _o.ID) + "', name: '" + (!_text.node.textContent ? "" : _text.node.textContent) + "',text:{text:'"
    ...
    myflow.init = function (c, o) {
        ...
        $.extend(true, myflow.config, o, {rawdata: o.restore});
        ...
        if (myflow.config.editable) {
            ...
            $('#myflow_save').click(function () {// 保存
                myflow.config.tools.save.call(myflow.config, getJson())
            });
        ...
        function restore(data){
            ...
            if (data.states) {
                for (var k in data.states) {
                    if(!_states[k]){
                        var rect = new myflow.rect( $.extend( true, {}, myflow.config.tools.states[data.states[k].etype], data.states[k]), _r, k);
