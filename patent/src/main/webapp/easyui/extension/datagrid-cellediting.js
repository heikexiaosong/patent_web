			...
			if (input){
				if (accepted){
					if (dg.datagrid('validateRow', cell.index)){
                        ...
                        dg.datagrid('refreshRow', cell.index);
	...
	function editCell(target, param){
		...
		if (col.editor){
			...
			if (input){
                if(input[0]['type']=='checkbox'){
                    ...
                    $(input).css('width','16px');
                    $(input).css('marginLeft','30px');
                }
				dg.datagrid('gotoCell', param);
				setTimeout(function(){
        ...
        $(document).off().on('click',function (e) {
            if($(document).find(this).length>0){
            	var $dg = this;
                var endRows = this.datagrid("getRows");
			...
			if (opts.clickToEdit){
				$(this).datagrid('editCell', {index:index,field:field});
                    ...
                    if (endCellEdit(this, true)){
                        ...
                        $(this).datagrid('getPanel').find('div.datagrid-body td').parent().removeClass('datagrid-row-selected');
                        $(this).datagrid('getPanel').find('div.datagrid-body td.datagrid-row-over').parent().addClass('datagrid-row-selected');
                    }
